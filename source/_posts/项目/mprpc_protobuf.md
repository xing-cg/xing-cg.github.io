...
int main()
{
    ...
    // 从send_str反序列化为一个LoginRequest对象
    LoginRequest reqB;
...
protobuf生成的类都会给每一个属性提供`set_xxx`的方法。
...
> 写到此，我们发现，不同的响应类存在重复的代码，即errcode和errmsg，可以抽象封装为一个ResultCode类型。
>
> ```protobuf
> message ResultCode
> {
...
可以看到，对于`repeated`类型，即列表类型，都生成了`add_xxxxxx`方法。往列表里增加一个元素，并返回该元素的地址。
...
默认是不会生成service服务类和rpc方法描述的。要生成，需要加option选项。

```protobuf
optio cc_generic_services = true;
```

上面这句表示：生成service服务类和rpc方法描述。
